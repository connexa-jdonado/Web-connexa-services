...
export default function ServiciosClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormError(tr('No se pudo enviar. Intentá de nuevo más tarde.', 'Could not send. Please try again later.'));
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>Servicios | Connexa Services</title>
        <meta name="description" content="Implementación OFSC, Zinier, consultoría FSM, integraciones, desarrollo a medida y soporte post go-live. Partner certificado Oracle y Zinier." />
              ...
              {!formSent ? (
                <form onSubmit={handleSubmit}>
                  <div className="form-row">
                    <div className="form-group"><label>{tr('Nombre *', 'Name *')}</label><input type="text" name="nombre" placeholder={tr('Tu nombre', 'Your name')} required /></div>
                    <div className="form-group"><label>{tr('Empresa *', 'Company *')}</label><input type="text" name="empresa" placeholder={tr('Nombre de la empresa', 'Company name')} required /></div>
                  </div>
                  <div className="form-group"><label>{tr('Email *', 'Email *')}</label><input type="email" name="email" placeholder="tu@empresa.com" required /></div>
                  ...
                  <div className="form-group"><label>{tr('Mensaje', 'Message')}</label><textarea name="mensaje" placeholder={tr('Contanos brevemente tu necesidad o proyecto…', 'Tell us briefly about your need or project…')}></textarea></div>
                  <button type="submit" className="btn-primary form-submit" disabled={sending}>{sending ? tr('Enviando…', 'Sending…') : tr('Enviar consulta', 'Send inquiry')}</button>
                  {formError && <p className="form-note" style={{ color: '#b91c1c' }}>{formError}</p>}
